...
inline ContextPrivate * xre_context_children_get(
...
inline struct XREContext const * xre_context_children_get_ro(
    struct XREContext const * context,
    char const * key
) {
    ContextPrivate const * ctx = TO_CONTEXT_PRIV(context);
    ContextPrivate const * child = xre_context_children_get(ctx, key);
